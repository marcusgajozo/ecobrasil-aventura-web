...
export const generateRandomMapsPaths = () => {
  const shuffledMaps = [...NAME_MAPS].sort(() => Math.random() - 0.5); // Embaralha os mapas
  ...
  const extraPathsCount = Math.floor(NAME_MAPS.length / 2); // Adiciona alguns caminhos extras

  for (let i = 0; i < extraPathsCount; i++) {
    const randomIndex = Math.floor(Math.random() * paths.length);
    const randomTarget =
      NAME_MAPS[Math.floor(Math.random() * NAME_MAPS.length)];
